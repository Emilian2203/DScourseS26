...
svm_parm_df1 <- tibble(cost      = c(0.25, 1, 4))
svm_parm_df2 <- tibble(rbf_sigma = c(0.25, 1, 4))
svm_parm_df  <- full_join(svm_parm_df1, svm_parm_df2, by = character())

rec_folds <- vfold_cv(income_train, v = 3)

svm_recipe <- recipe(high.earner ~ education + marital.status + race + workclass + occupation + relationship + sex + age + capital.gain + capital.loss + hours, data = income_train) %>%
